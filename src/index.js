...
function displayJSON(json){     //displayJSON() parses the data and inyects it in the template
    ...
    for(let i = Object.keys(json.articles["0"]).length - 1; i >= 0; i--){
        if (values[i][0]["displayES"] || values[i][0]["displayEN"]){    //check for each language
            let div = document.createElement("a");
            div.classList.add("item", `${keys[i]}`);

            let filteredLang = [];
            language.forEach((lang)=>{
                 if (values[i][0]["display"+lang]){
...
async function getJSON() {  //getJSON() retrieves the data
    ...
    //const response = await fetch('./src/data/hierarchy.json'); //code for local tests.
